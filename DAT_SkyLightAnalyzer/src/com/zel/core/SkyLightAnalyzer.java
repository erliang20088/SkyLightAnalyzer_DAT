...
public class SkyLightAnalyzer {
	...
	public boolean isValidWord(String tempWord, int begin,
		...
		isValid = (!StringUtil.isAllAlpha(tempWord) && !StringUtil
				.isAllDigital(tempWord))
				|| ((StringUtil.isAllAlpha(tempWord)
						&& (begin - 1 > -1
								&& (!StringUtil.isAlpha(this.content
										.charAt(begin - 1))) || begin - 1 <= -1) && (currentOrFoundPos + 1 < array_length
						&& (!StringUtil.isAlpha(this.content
								.charAt(currentOrFoundPos + 1))) || currentOrFoundPos + 1 == array_length)) || (StringUtil
						.isAllDigital(tempWord)
						&& (begin - 1 > -1
			...
			if (branch == null) {// 没查着，说明trie树中没有该char,故略过,进入下一个char
				...
				case 2:// 继续往下循找
					for (; (current + 1) < array_length; current++) {// 数组的第二个下标向下遍历
						endFor = false;
						temp_position = branch.getBase_value()
								+ this.content.charAt(current + 1);
						Branch temp_branch = forest.base_array[temp_position];
						if (temp_branch != null && temp_branch.getFrontPosition()==branch.getPosition() && temp_branch.getCharNode()==this.content.charAt(current + 1)) {// 说明找到下一个char
							switch (branch.getStatus()) {
							case 1:
	...
	public List<String> getSplitResult() {
		...
		for (begin = 0; begin < array_length; begin++) {
			...
			if (!found_new) {
				list
						.add(this.content.substring(begin,
								(begin + 1 <= array_length ? begin + 1
										: array_length)));
			}
		}
		return list;
